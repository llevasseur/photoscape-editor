...
def find_nav_index(li_list, txt):
    ...
    for i in range(0, len(li_list)):
        index = i
        try:
            found = li_list[i].find_element(
                By.XPATH, f'.//span[contains(text(), "{txt}")]'
            )
            return index
...
def fetch_date(driver):
    # DATE
    game_info = driver.find_element(
        By.XPATH, './/div[ contains( @class, "GameInfo__Meta" ) ]'
    )
...
def get_game_preview(data, site):
    global error
    ...
    try:
        ...
        loading_thread = threading.Thread(target=h.loading_animation)
        loading_thread.start()

        element_present = EC.presence_of_element_located(
            (By.XPATH, './/div[ contains( @class, "Gamestrip" ) ]')
        )
    ...
    # Fetch Gamestrip
    gamestrip = driver.find_element(
        By.XPATH, './/div[ contains( @class, "Gamestrip" ) ]'
    )

    # Fetch h2_list which are the two teams
    h2_list = gamestrip.find_elements(
        By.XPATH, './/h2[ contains( @class, "ScoreCell__TeamName" ) ]'
    )

    # Fetch records
    records = gamestrip.find_elements(
        By.XPATH, './/div[ contains( @class, "Gamestrip__Record" ) ]'
    )
    ...
    # Set data
    data["CANUCKS"] = {"TEAM": "VAN", "HOME": home, "RECORD": van_record}

    data["OTHER"] = {"TEAM": other_team, "RECORD": other_record}

    # Extract date using helper.get_date_json
    # Returns date_file
    date_file = h.get_date_json(data, parsed_date)

    if h.valid_date(date_file):
        return date_file
...
def get_final_score_data(data, site):
    ...
    nav = driver.find_element(
        By.XPATH, './/nav[ contains( @class, "Nav__Secondary" ) ]'
    )
    ...
    try:
        element_present = EC.presence_of_element_located(
            (By.XPATH, './/div[ contains( @class, "Gamestrip__Team--away" ) ]')
        )
        WebDriverWait(driver, timeout).until(element_present)
    ...
    away_content = away.find_element(
        By.XPATH, './/div[ contains(@class, "Gamestrip__TeamContent" ) ]'
    )
    away_team = away_content.find_element(By.TAG_NAME, "h2").text
    away_score = away_content.find_element(
        By.XPATH, './/div[ contains( @class, "Gamestrip__Score" ) ]'
    ).text

    home_obj = driver.find_element(
        By.XPATH, './/div[ contains( @class, "Gamestrip__Team--home" ) ]'
    )
    home_content = home_obj.find_element(
        By.XPATH, './/div[ contains( @class, "Gamestrip__TeamContent" ) ]'
    )
    home_team = home_content.find_element(By.TAG_NAME, "h2").text
    home_score = home_content.find_element(
        By.XPATH, './/div[ contains( @class, "Gamestrip__Score" ) ]'
    ).text
    ...
    stats_table = driver.find_element(
        By.XPATH, './/section[ contains( @class, "TeamStatsTable" ) ]'
    )
    stats = stats_table.find_element(
        By.XPATH, './/div[ contains( @class, "Table__Scroller" ) ]'
    )
    tbody = stats.find_element(
        By.XPATH, './/tbody[ contains( @class, "Table__TBODY" ) ]'
    )
...
def get_box_score_data(data, site):
    ...
    try:
        ...
        goal_section = driver.find_element(
            By.XPATH, './/div[ contains( @class, "tabs__content" ) ]'
        )

        # Get each tbody w class Table__TBODY
        tbody_list = goal_section.find_elements(
            By.XPATH, './/tbody[ contains( @class, "Table__TBODY" ) ]'
        )
        ...
        if live:
            ...
            period_list = goal_section.find_elements(
                By.XPATH, './/thead[ contains( @class, "Table__THEAD" ) ]'
            )
            period_title = (
                period_list[1]
                .find_element(
                    By.XPATH, './/th[ contains( @class, "Table__TH" ) ]'
                )
                .text.lower()
            )
        ...
        for i in range(0, len(tbody_list)):
            ...
            tr_list = tbody_list[i].find_elements(
            ...
            if p > 2:
                ...
                if p == 4:
                    ...
                    # Create data objects
                    data["CANUCKS"].append(
                        {"PERIOD": "SHOOTOUT", "SCORERS": []}
                    )
                    canucks_so_index = len(data["CANUCKS"]) - 1

                    data["OTHER"].append({"PERIOD": "SHOOTOUT", "SCORERS": []})
            ...
            for j in range(0, len(tr_list)):
                ...
                if so:
                    ...
                    shooter = h.extract_second_word(td_list[1].text).upper()

                    # Check if the shootout counter changes
                    # Update data if so
                    if home == "AWAY":
                        if td_list[2].text != str(van_so):
                            data["CANUCKS"][canucks_so_index].get(
                                "SCORERS"
                            ).append(shooter)
                            van_so += 1

                        elif td_list[3].text != str(other_so):
                            data["OTHER"][other_so_index].get("SCORERS").append(
                                shooter
                            )
                            other_so += 1
                    ...
                    if not test == "No Goals Scored":
                        ...
                        if home == "AWAY":
                            if td_list[3].text != str(van_score):
                                ...
                                h.get_scorer_and_assistors(
                                    obj, td_list[2].text, False
                                )
...
def fetch_game_preview(site):
    data = {}

    # Update data structure and get date to be used as dir name
    date_file = get_game_preview(data, site)
    ...
    with open(
        documents_folder + f"/NT/games/{ date_file }/game-day.json", "w"
    ) as json_file:
...
def fetch_box_score(site):
    ...
    # Dump data to game-day.json
    with open(
        documents_folder + f"/NT/games/{ date_file }/box-score.json", "w"
    ) as json_file:
        json.dump(data, json_file, indent=4)
